...
\item The computation of an element's rank before it is enqueued into
  ...
  in a PIFO.\footnote{Recall from \S\ref{s:transactions} that any state
    visible on the router after processing $N$ consecutive packets is
    identical to a serial execution of the transactions across the $N$
    packets in order of packet arrival.}
...
max-min allocation of link capacity across flows\footnote{We define a flow to
be any set of packets sharing
