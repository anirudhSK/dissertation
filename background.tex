%TODO: Need to proof read this section
...
%distributed aggregations that is relavant to our linear-in-state formalism.
...
%%This is because
%%there is no  indefinitely while
...
%TODO: Make sure everything in this chapter is past tense by default
...
receiving this message, the arbiter specifies the times at which the end
host must transmit and the network path that the end host's packets must follow
during those times. 
...
size, until the centralized arbiter becomes a bottleneck.
Microbenchmarks that stress the Flowtune arbiter alone show that an arbiter running on a
64-core server can manage a network with an aggregate capacity of 184
Tbit/s~\cite{flowtune}. However, the largest network that a
...
by a programmable router chip---but at a significant cost to scalability.
