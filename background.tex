...
By the mid 90s, software could no longer keep up with the demand for higher
...
operators. Extensions to the API included the ability to maintain simple state
machines in the data plane~\cite{fast}, the ability to perform counter-based
network measurements using sketches~\cite{opensketch}, and the ability to be
independent of the actual protocol in the packet's headers~\cite{pof}.
%We compare against fast in stateful packet processing a few subsections later
...
this approach, the network's routers were divided into two classes. The {\em
...
CPUs.  Using a general-purpose CPU for programming the edge routers allowed
them to be far more programmable than the restricted OpenFlow API for routers
would have allowed. Open Virtual Switch~\cite{openvswitch} is one well-known
example of an edge router; it runs within the hypervisor on an end host and
connects together multiple virtual machines on a single end host to the
network.  More recently, increasing performance requirements on the edge have
led to moving the functionality of such virtual switches on to an
FPGA~\cite{daniel_firestone_nsdi}.

Taken to its logical extreme, the edge router could be the
end host itself. Hence, we include approaches that use the end host to achieve
network flexibility within our discussion of edge-based approaches.


While quite a bit of programmable networking functionality can be carried out
at the edge (\eg network virtualization, access control, security policies
etc.), there is a benefit to moving some programmable networking functionality
into the core for improving both network performance and visibility. As
discussed earlier in Chapter~\ref{chap:intro}, network monitoring from end
hosts (or the edge closest to the end hosts) is indirect and inaccurate
relative to directly instrumenting routers, especially if the problem is buried
deep inside the network. Further, many proposals for improving network
performance rely on support from the core of the network~\cite{pFabric, pias,
d3, rcp, xcp, detail, conga, letflow}. Thus, the absence of a programmable
network core leaves significant performance on the table and complicates
network debugging.
...
program which packet a router link must schedule next, and the ability to
...
of such programmable router chips.
...
approach has been benchmarked on is a network of 40 10-Gbit/s servers connected
to a single top-of-rack router~\cite{flexplane}.
...
%TODO: Mention somewhere earlier that it's a commodity server

%% resume here
%% TODO: The end-host part is very repetitive
\subsection{End-host-based programmability}
Several recent approaches to network programmability provide programmability
from the end hosts. For instance, Eden~\cite{eden} provides a programmable data
plane using commodity routers by programming end hosts alone.  Tiny Packet
Programs (TPP)~\cite{tpp} allow end hosts to embed small programs in packet
headers, which are then executed by the router in a style similar to
...
end hosts alone~\cite{netpoirot, minlan-snap, dapper-sosr, trumpet,
azure-smartnic}.

End host solutions are necessary for application context that is not available
...
%TODO: Repeating for a third time. Remove something.
...
%TODO: XCP router support be very clear.

%TODO: One possibility sounds off
...
This is an interesting approach that tacitly presupposes the existence of a
...
cache eviction rate (\Sec{eval}).  \TheSystem also allows users to perform a
...
%%\TheSystem shares some language
...
\Para{SONATA.} SONATA~\cite{streaming-monitoring} is a measurement system that sources its
