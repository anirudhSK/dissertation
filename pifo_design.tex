...
introduced previously in Chapter~\ref{chap:domino}. Domino allows us to express
these transactions in a high-level domain-specific language, using a compiler to
compile the transactions down to a pipeline of hardware instructions or atoms.
 One of the examples used to evaluate Domino in \S\ref{s:eval} is the STFQ transaction from
...
and LSTF (\S\ref{ss:lstf}), can all be expressed as Domino programs and
then compiled to a router pipeline with a sufficiently expressive atom.\an{An
important restriction in Domino is the absence of loops, which precludes rank
computations containing a loop with an unbounded iteration count. We have not,
however, encountered a scheduling or shaping transaction requiring this
capability.}
