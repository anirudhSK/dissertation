...
\emph{state} vector. $f$ mathematically formalizes incremental computations over packet streams,
...
of packets, similar to a fold function. Additionally, we stay that a merge function successfully merges an aggregation function if Equations~\ref{eqn:start_condition}--~\ref{eqn:merge_condition} hold. 
...
The closure graph thus has the following property: given a sequence of packets $\{p_1, \ldots p_k\}$, start at the identity function and, on the $i$th step, follow the edge corresponding to each packet $p_i$. The ending vertex of this process is the iterated function that captures the effect of this packet sequence on any starting state. We say that this ending vertex is the result of \emph{updating the iterated function} by the given sequence of packets. To update the iterated function stored on the router given a new packet, we simply follow the edge labeled with that packet.
...
\end{theorem}
\begin{proof}
We can represent each iterated function $f_i$ in $n2^n$ bits, using a list of $2^n$ $n-$bit numbers, where the $k$th item is $f_i(k)$. The router stores this representation. For each new packet $p$, the router computes the update to each number in the list, \ie $f_i(k)$ is updated to $f(f_i(k), p)$. Upon a merge, the router sends this representation to the backing store, which can then evaluate $f_i(s_d)$, where $s_d$ is the value stored in the backing store.
\end{proof}

Note that the router needs to tell the backing store what iterated function to apply on the value it currently has. Each iterated function in the closure graph is a possible candidate for what the router needs to tell the backing store, so the router needs to convey at least $\log |G(f)|$ auxiliary bits for the merge to be possible.

As an example, consider a simple counter, where $f(s, p) = s + 1$ and $s_0 = 0$. If the counter has $n$ bits, the closure graph is a ring of size $2^n$, where vertex $i$ encodes the iterated function $f_i(s) = s + i$. Note that all edges from vertex $i$ point to vertiex $i+1$. For every observed packet, the router walks one step further around the ring. This is implemented efficiently via adding 1 to an $n$-bit counter. After $K$ packets, the iterated function on the router is $f_{k}$, where $k = K \mod 2^n$. Upon merging, this is sent to the backing store, which then knows to add $k$ to the existing backing store value.
...
%TODO: Is U a function or an informal update or something else?
...
\end{proof}
There are $2^{n2^n}$ such packet sequences, meaning that the closure graph has at least $2^{n2^n}$ distinct vertices, and thus the number of auxiliary bits needed is at least $\log 2^{n2^n} = n2^n$. 
\end{proof}
...
Then, since we must enumerate every possible $p$-bit packet (\ie $2^p$ packets in total), the total runtime is:
\[ O(\text{\# iters} \cdot \text{\# packets} \cdot \text{\# time to test if function has been seen}) = O\left(2^{n2^n} \cdot 2^p \cdot n2^n\right) \]
\end{proof}

This algorithm is intractable in practice. However, a polynomial time algorithm is unlikely
to exist.
...
equation~\ref{eqn:projection_condition}. However, we can show that there must then also be a
...
$P_{small}$. If not, running $f$ on the substream of packets until
...
Now we know that checking whether $m$ successfully merges $f$ is actually decidable because
we can carry out the check on a finite number of packet streams (all streams of length
up to $M$).
...
that a \emph{given} merge function successfully merges a \emph{given} aggregation function.
...
First, we observe that $MERGE(s \oplus t', 0, s \oplus t', OR, I, 0)$ tells us whether statement~\ref{eqn:merge} is true:
\begin{align}
