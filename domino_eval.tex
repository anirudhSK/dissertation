...
\begin{table}[!t]
\begin{small}
  \begin{tabular}{|p{0.15\columnwidth}|p{0.34\columnwidth}|p{0.10\columnwidth}|p{0.06\columnwidth}|p{0.08\columnwidth}|p{0.04\columnwidth}|p{0.04\columnwidth}|}
\hline
...
\hline
Priority computation for weighted fair queueing (Chapter~\ref{chap:pifo}) & Compute packet's virtual start time using finish time of last packet in that flow. & Nested & 4, 2 & Ingress & 29 & 87 \\
\hline
...
Table~\ref{tab:templates}~(\S\ref{domino_ss:compiler}).  We conclude with some lessons
for programmable router design~(\S\ref{ss:lessons}).
...
programming scheduling using push-in first-out queues, as described later in
Chapter~\ref{chap:pifo}.
...
As Table~\ref{tab:algos} shows, the choice of stateful atom determines
 what algorithms can run on a router. For instance, with only the
...
In summary, for a router designer, the minimum delay on the critical path of atoms
is the most important metric to optimize. The most programmable line-rate
routers will have the highest density of useful stateful functionality
