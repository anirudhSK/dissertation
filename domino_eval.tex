...
\Para{Algorithms that are not supported by our atoms.} In addition to CoDel,
...
One class of algorithms requires operations on real numbers.  CoDel belongs to
...
Another class of algorithms are algorithms that fall just outside the boundary
of our atoms, as determined by our (conservative) compiler. An example is the
two-color three-rate meter~\cite{trTCM}. We have found that explaining {\em
why} an algorithm cannot compile to a target with a particular atom is
challenging. So far, we have had to manually  pare down an algorithm until we
are left with a problematic ``core'' of the algorithm that does not map to the
underlying atom. This is similar to how some SAT solvers provide an
unsatisfiable core so that a user can understand why a particular input to a
SAT solver is unsatisfiable~\cite{unsat_core}. Automating this process and
providing guidance to the programmer on how they should fix their algorithm so
that it compiles is an interesting direction for future work.
%TODO: Explain why it fails

%% but note that in
%%subsequent work (described in \S\ref{sec:hardware-feasibility}) we found that
%%an atom supporting a multiplication followed by an addition is feasible at 1
%%GHz speeds.  
