\section{Evaluation}
\label{s:eval}
%TODO: Needs one careful proof reading pass
...
meter~\cite{trTCM} (trTCM). We have found that explaining {\em why} an algorithm cannot
compile to a target with a particular atom is challenging. Next, we describe
...
example of this procedure applied to trTCM.
...
two token buckets filling at the peak and committed rate, and last\_time is the
last time either of the two buckets was updated.
...
unsatisfiable core~\cite{unsat_core}. This unsatisfiable core is a subset of
the clauses in the input formula that is still unsatisfiable and helps a user
understand why a particular input formula is unsatisfiable. Because our
SKETCH-based code generator uses a SAT solver internally~\cite{sketch_asplos},
we hope to draw on algorithms for finding unsatisfiable cores~\cite{unsat_core}
to automate the paring down process in Domino.
